# d:\code\spring2026\graphic\cg-lib\src\Work4_xz_jc\physics.py
import taichi as ti
from .config import *
...
def render(pixels: ti.template(), width: ti.i32, height: ti.i32):
    ...
    for i, j in pixels:
        ...
        for s in range(MSAA_SAMPLES):
            # 随机抖动采样位置
            jitter_x = (ti.random() - 0.5) / width
            jitter_y = (ti.random() - 0.5) / height
            
            # 将屏幕坐标转换为标准化设备坐标
            u = (i + 0.5 + jitter_x) / width
            v = (j + 0.5 + jitter_y) / height
        ...
        # 平均多次采样的结果
        averaged_color = accumulated_color / MSAA_SAMPLES
        
        # 设置像素颜色，限制在[0,1]范围内
        pixels[i, j] = ti.Vector([
            min(1.0, averaged_color.x),
            min(1.0, averaged_color.y), 
            min(1.0, averaged_color.z)
        ])